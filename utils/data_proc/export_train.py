...
def generate_database_xia(bvh_path, output_path, window, window_step, dataset_config='xia_dataset.yml'):
    ...
    for i, item in enumerate(bvh_files):
        ...
        uclip = motion_and_phase_to_dict(process_file(item, divider=divide_clip_xia, window=window, window_step=window_step, downsample=downscale,
        ...
        if test_cnt[content_style] < content_test_cnt[content]:
            ...
            clips = motion_and_phase_to_dict(process_file(item, divider=divide_clip_xia, window=window, window_step=window_step, downsample=downscale,
                                                          skel=skel, divide=True),
                                             style_name_to_idx[style],
                                             {"style": style, "content": content})
            train_inputs += clips
    
    # for i, clip in enumerate(train_inputs):
    #     motion = np.transpose(clip["motion"][:, :-4])
    #     save_bvh_from_network_output(motion, output_path=pjoin('./train_result/', '{}_{}_{}.bvh'.format(i, clip["meta"]["style"], clip["meta"]["content"])))
