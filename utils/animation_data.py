...
class AnimationData:
    ...
    def __init__(self, full, skel=None, frametime=1/30):
        ...
        assert self.rotations.shape[1] == len(self.skel.topology)
    ...
    def get_root_posrot(self):
        return np.concatenate((self.rt_pos, self.rt_rot), axis=1).transpose(1,0)
    ...
    def get_foot_contact(self, transpose=True):
        if transpose:
    ...
    def get_joint_rotation(self):
        ...
        return rotations  # [Jo * 4 + 3 + 1, T]

    def get_joint_position(self):
        pos3d = forward_rotations(self.skel, self.rotations, trim=True)[:, 1:]  # [T, J - 1, 3]
        pos3d = pos3d.reshape((len(pos3d), -1))  # [T, (J - 1) * 3]
        return pos3d  # [(J - 1) * 3 + 3 + 1, T]
    
    def get_joint_velocity(self):
        # positions = forward_rotations(self.skel, self.get_original_rotations(), trim=True)[:, 1:]  # [T, J - 1, 3]
        positions = forward_rotations(self.skel, self.rotations, trim=True)[:, 1:]
        episode_length = positions.shape[0]
        velocity = positions[1:] - positions[:-1]  # [T - 1, J - 1, 3]
        last_velocity = (2 * velocity[-1] - velocity[-2])[np.newaxis, ...]
        return np.concatenate((velocity, last_velocity), axis=0).reshape(episode_length, -1)
